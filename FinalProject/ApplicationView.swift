...
protocol ModelContainer: AnyObject {
    var internships: [Internship] { get set }
}
...
class ApplicationView: UIViewController {
    ...
    @objc func addInternship() {
        ...
        internship = Internship(id: UUID(), CompanyName:  companyName.text, InternshipTitle: internTitle.text, InternshipDescription: internDescription.text, ApplicationDeadline: appDeadline.text, ApplicationStatus: appStatus.text, ApplicationLink: appLink.text, InterviewStatus: interviewStatus.text, InterviewDate: interviewDate.text, InternshipLocation: internLocation.text)

        NotificationCenter.default.post(name: Notification.Name("internship"), object: internship)
